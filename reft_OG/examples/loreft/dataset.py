...
class LoReftSupervisedDataset(ReftDataset):
    ...
    def preprocess(self, kwargs):
        ...
        elif 'indiv' in self.task:
            self.data_path = self.data_path
            self.task = self.task.split("_")[0]
    ...
    def tokenize(self, data_item):
        ...
        if self.task == "commonsense":
            if "instruction" in data_item:
                ...
                base_prompt = self.task_prompt_template % (data_item['passage'] + "\n"+ data_item['question'] + "\nAnswer format: true/false")
                # data_item['question'] 
                # + "\n\nAnswer format: true/false\n\n" + self.trigger_tokens
                if data_item["answer"]:
                    ans = "true"
                    ...
                    ans = "false"
                base_input = base_prompt + self.trigger_tokens + ans + self.tokenizer.eos_token
