...
def compute_metrics(
    ...
    if "Meta-Llama-3-8B" in tokenizer.name_or_path: # pretty bad workaround for llama-3, forgive me
        terminators = [
            ...
            if task == "glue":
                ...
                for id, pred in zip(inputs["id"].tolist(), actual_preds):
                    ...
                    if not is_base:
                        ...
                        assert base_model_trigger != None
                        raw_generation = pred.split(base_model_trigger)[-1].strip().rstrip().lower()
                    ...
                    if "commonsense" in task:
                        ...
                        if "indiv" in task:
                            ...
                            if answer.strip() in generation.strip():
                                correct = 1
                                correct_count += 1
                            else: 
                                correct = 0
                    ...
                    if task not in ["alpaca", "instruct", "ultrafeedback", "ultrafeedback_pair"]:
                        ...
                        instruction = example["question"] if task == "gsm8k" or "indiv" in task else example["instruction"]
                        generations += [{
